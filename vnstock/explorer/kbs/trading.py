...
class Trading:
    ...
    def trade_history(
        ...
        """
        if not self.symbol:
            raise ValueError("Symbol is required for trade_history method.")

        url = f'{_STOCK_TRADE_HISTORY_URL}/{self.symbol}'
        ...
        df = df.rename(columns=_TRADE_HISTORY_MAP)
        ...
        if show_log or self.show_log:
            logger.info(
                f'Truy xuất thành công {len(df)} bản ghi lịch sử giao dịch cho {self.symbol}.'
            )
