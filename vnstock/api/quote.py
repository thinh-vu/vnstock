...
class Quote(BaseAdapter):
    ...
    def __init__(
        ...
        if source.lower() not in [s.lower() for s in DataSources.ALL_SOURCES[:3]]:
            raise ValueError(f"Lớp Quote chỉ nhận giá trị tham số source là {', '.join(DataSources.ALL_SOURCES[:3])}.")        
