...
def batch_normalize_vietnamese_fields(texts: list[str], **kwargs) -> Dict[str, str]:
    """
...
def convert_derivative_symbol(symbol: str, reference_date: date = None) -> str:
    ...
    if not underlying_code:
        raise ValueError(
            f"Unknown underlying asset for symbol {symbol}. Supported prefixes: {', '.join(underlying_map.keys())}"
        )
