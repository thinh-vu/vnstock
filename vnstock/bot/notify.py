...
class Messenger:
    ...
    def _validate(self):
        ...
        if self.platform == "slack":
            ...
            if not (token_str.startswith("xoxb-") or token_str.startswith("xoxp-")):
                raise ValueError("Invalid Slack token. Must start with xoxb- or xoxp-")
            if not self.channel or not self.channel.startswith("#"):
                raise ValueError("Slack channel must start with # (e.g., #general)")
        ...
        # Discord validation
            ...
            if not self.webhook_url:
                raise ValueError("Discord requires webhook_url parameter")
        ...
        # Lark validation
            ...
            if self.channel is not None:
                raise ValueError("Lark messaging does not require a channel")
    ...
    def _telegram_photo(self, message: str, file_path: str) -> requests.Response:
        ...
        url = f"https://api.telegram.org/bot{self.token_key}/sendPhoto"
    ...
    def _lark_message(self, message: str, payload: Optional[Dict] = None) -> Dict:
        """
