...
class TestVCIListingComprehensive:
    ...
    def test_symbols_by_exchange(self, lang):
        ...
        df = listing.symbols_by_exchange(lang=lang, show_log=False)
    ...
    @pytest.mark.parametrize(
        "group", ["VN30", "VNMIDCAP", "VNSMALLCAP", "VN100", "VNALLSHARE"]
    )
    def test_symbols_by_group(self, group):
        ...
        try:
            df = listing.symbols_by_group(group=group, show_log=False)
    ...
    def test_listing_data_consistency(self):
        ...
        hose_df = listing.symbols_by_exchange(lang="vi", show_log=False)
        ...
        if "type" in hose_df.columns:
            ...
            assert hose_symbols.issubset(all_symbols), "HOSE symbols not in all_symbols"
