...
import pandas as pd
from vnstock3.explorer.fmarket.fund import Fund, convert_unix_to_datetime
...
class TestFund(unittest.TestCase):
    ...
    @patch('vnstock3.explorer.fmarket.fund.requests.post')
    ...
    @patch('vnstock3.explorer.fmarket.fund.requests.get')
    @patch('vnstock3.explorer.fmarket.fund.Fund.filter')
    ...
    def test_dynamic_fund_short_names(self):
        for short_name in self.short_names:
            with patch('vnstock3.explorer.fmarket.fund.requests.post') as mock_post:
                mock_response = MagicMock()
                mock_response.status_code = 200
                mock_response.json.return_value = {
                    "data": {
