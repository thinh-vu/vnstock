...
- 28-04-2026: Cập nhật phiên bản chính thức `4.0.0` - thế hệ Vnstock thứ 4 với giao diện người dùng thống nhất và nâng cấp API.
- 16-01-2026: Cập nhật phiên bản `3.4.0` - giới thiệu nguồn dữ liệu KBS dùng được trên mọi môi trường thiết bị mà không bị hạn chế.
...
👉 Để hiểu rõ hơn về vnstock và hướng dẫn sử dụng toàn diện, bạn có thể truy cập [vnstocks.com](https://vnstocks.com/docs), sao chép tài liệu hướng dẫn và Agent Skills để trợ lý AI của bạn có thể viết code chính xác thông qua Agent Guide được chia sẻ qua Github.

<div id="badges" align="center">
  <a href="https://vnstocks.com/docs/vnstock" target="_blank">
    <img src="https://img.shields.io/badge/Hướng%20dẫn%20nhanh%20-View-blue?style=for-the-badge&logo=book" alt="Tài liệu hướng dẫn Vnstock Badge"/>
  </a>
  <a href="https://vnstocks.com/docs/vnstock" target="_blank">
    <img src="https://img.shields.io/badge/AI%20Agent%20Guide%20-Clone-green?style=for-the-badge&logo=book" alt="Tài liệu hướng dẫn Vnstock Badge"/>
  </a>
</div>
...
> 💡 **Lưu ý**: Các hướng dẫn (Agent Skills) cho AI Agents của dự án được lưu trữ tại [vnstock-agent-guide](https://github.com/vnstock-hq/vnstock-agent-guide/). Bạn có thể clone kho lưu trữ này về để sử dụng với Google Antigravity (mặc định) hoặc bất kỳ coding agent nào khác nhằm giúp AI hiểu và viết code Vnstock chính xác nhất.
...
## 8.3. 🌟 Giao diện Hợp nhất (Unified UI) - Bước đột phá của Vnstock v4+ (Khuyên dùng)

Vnstock v4+ mang đến **Giao diện làm việc hợp nhất (Unified UI)**, được thiết kế với tư duy hướng đến tương lai và loại bỏ hoàn toàn sự phức tạp khi sử dụng dữ liệu từ nhiều nguồn khác nhau. Đây là điểm khác biệt lớn nhất và là bước đột phá so với các phiên bản trước!

**🎯 Tại sao bạn nên dùng Unified UI?**
- **Chuyển đổi tư duy**: Bạn không cần phải bận tâm việc hàm này thuộc `vci`, `kbs` hay `msn`. Bạn chỉ cần quan tâm đến *Nhóm dữ liệu (Domain)*: `Market` (Thị trường), `Reference` (Tham chiếu), `Fundamental` (Cơ bản).
- **Độ ổn định cao nhất**: Vnstock tự động định tuyến đến API tốt nhất. Nếu một nguồn bị lỗi trong tương lai, tác giả sẽ chuyển sang nguồn thay thế để **hạn chế thay đổi cấu trúc code hiện tại của bạn**.
- **Tự động tối ưu**: Các tác vụ gọi API đều được ẩn giấu qua Facade, tự động quản lý request và log thông qua AI/trình quản lý thông minh.

**💻 Ví dụ minh hoạ với Unified UI:**

```python
from vnstock import Market, Reference, Fundamental

# Khởi tạo các nhóm (domain) dữ liệu
market = Market()
ref = Reference()
fund = Fundamental()
...
> **🔍 Khám phá cấu trúc API**: Để xem toàn bộ sơ đồ các hàm và nguồn dữ liệu (provider) đang được hệ thống ánh xạ, bạn có thể gọi hàm sau:
```python
...
## 8.4. Nạp thông qua các class tổng hợp
...
## 8.5. Nạp các lớp tính năng riêng lẻ theo nguồn dữ liệu cố định
...
## 8.10. Truy xuất thông tin công ty
...
## 8.11. Truy xuất báo cáo tài chính
...
> 💡 **Note**: The instruction files (Agent Skills) for AI Agents of this project are located at [vnstock-agent-guide](https://github.com/vnstock-hq/vnstock-agent-guide/). You can clone this repository to use with Google Antigravity (default) or any other coding agent to help them understand and write accurate Vnstock code.
...
## 8.3. 🌟 Unified UI - The Breakthrough of Vnstock v4+ (Recommended)

Vnstock v4+ introduces the **Unified UI**, designed with a forward-thinking paradigm that entirely eliminates the complexity of using data from various sources. This is the biggest difference and breakthrough compared to previous versions!

**🎯 Why should you use Unified UI?**
- **Paradigm Shift**: You no longer need to worry whether a function belongs to `vci`, `kbs`, or `msn`. You only need to care about the *Data Domain*: `Market`, `Reference`, `Fundamental`.
- **Maximum Resilience**: Vnstock automatically routes to the best available API. If a source fails in the future, the author will switch to a fallback source to **minimize changes to your existing code structure**.
- **Auto-Optimization**: All API calls are hidden behind a Facade, which automatically manages requests, rate limits, and logging via AI/smart managers.

**💻 Unified UI Code Example:**
...
> **🔍 Explore API Structure**: To view the full map of functions and their corresponding data providers, you can call:
```python
from vnstock import show_api
show_api()
```

## 8.4. Import from Specific Data Providers
...
## 8.6. Historical Prices & Trading Stats
