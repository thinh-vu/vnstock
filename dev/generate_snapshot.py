...
def main():
    ...
    diversity_tasks = [
        {
            "n": "Reference().company('{}').info()",
            "m": "Company Reference (KBS)",
            "f": lambda t: lambda: Reference().company(t).info(),
        },
        {
            "n": "Reference().company('{}').shareholders()",
            "m": "Company Reference (KBS)",
            "f": lambda t: lambda: Reference().company(t).shareholders(),
        },
        {
            "n": "Reference().company('{}').insider_trading()",
            "m": "Company Reference (KBS)",
            "f": lambda t: lambda: Reference().company(t).insider_trading(),
        },
        {
            "n": "Reference().company('{}').news()",
            "m": "Company Reference (KBS)",
            "f": lambda t: lambda: Reference().company(t).news(),
        },
        {
            "n": "Reference().company('{}').events()",
            "m": "Company Reference (KBS)",
            "f": lambda t: lambda: Reference().company(t).events(),
        },
        {
            "n": "Reference().company('{}').officers()",
            "m": "Company Reference (KBS)",
            "f": lambda t: lambda: Reference().company(t).officers(),
        },
        {
            "n": "Reference().company('{}').subsidiaries()",
            "m": "Company Reference (KBS)",
            "f": lambda t: lambda: Reference().company(t).subsidiaries(),
        },
        {
            "n": "Reference().company('{}').ownership()",
            "m": "Company Reference (KBS)",
            "f": lambda t: lambda: Reference().company(t).ownership(),
        },
        {
            "n": "Reference().company('{}').capital_history()",
            "m": "Company Reference (KBS)",
            "f": lambda t: lambda: Reference().company(t).capital_history(),
        },
    ]
    ...
    for task in diversity_tasks:
        ...
        if df is not None:
            ...
            with open(
                f"assets/data/schemas/{filename}.json", "w", encoding="utf-8"
            ) as f:
                json.dump(meta, f, indent=2, ensure_ascii=False)
        else:
            print(
                f"WARNING: Could not find non-empty data for {task['n'].format('UNIVERSE')}"
            )

    # 2. Static / One-off tasks
    static_tasks = [
        {
            "n": "Reference().equity.list()",
            "m": "Equity Reference (KBS)",
            "f": lambda: Reference().equity.list(),
        },
        {
            "n": "Reference().industry.list()",
            "m": "Industry Reference (VCI)",
            "f": lambda: Reference().industry.list(source="vci"),
        },
        {
            "n": "Market().quote('SSI')",
            "m": "Market Global Quote (KBS)",
            "f": lambda: Market().quote("SSI"),
        },
        {
            "n": "Market().equity('SSI').ohlcv()",
            "m": "Equity Market (KBS)",
            "f": lambda: Market().equity("SSI").ohlcv(),
        },
        {
            "n": "Market().index('VN30').ohlcv()",
            "m": "Index Market (KBS)",
            "f": lambda: Market().index("VN30").ohlcv(),
        },
    ]
        ...
        for ticker in tickers:
            for statement in [
                ...
                try:
                    ...
                    meta = get_metadata(
                        df, notation, f"Fundamental ({group_name})", ticker_used=ticker
                    )
                    if meta:
                        all_snapshots[notation] = meta
                        filename = (
                        ...
                        with open(
                            f"assets/data/schemas/{filename}.json",
                            "w",
                            encoding="utf-8",
                        ) as f:
                            json.dump(meta, f, indent=2, ensure_ascii=False)
    ...
    print(
        f"Success! Snapshot generated in docs/SNAPSHOT.md for version {version} at {timestamp}."
    )
